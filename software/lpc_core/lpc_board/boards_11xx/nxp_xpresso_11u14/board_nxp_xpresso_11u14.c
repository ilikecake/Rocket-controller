...
static void Board_LED_Init(void)
{
	...
	Chip_GPIO_WriteDirBit(LPC_GPIO_PORT, 1, 19, true);
	Chip_GPIO_WriteDirBit(LPC_GPIO_PORT, 0, 20, true);

	Chip_GPIO_WritePortBit(LPC_GPIO_PORT, 1, 19, true);
	Chip_GPIO_WritePortBit(LPC_GPIO_PORT, 0, 20, true);
}
...
void Board_LED_Set(uint8_t LEDNumber, bool On)
{
	if(LEDNumber == 0)
	{
		Chip_GPIO_WritePortBit(LPC_GPIO_PORT, 1, 19, On);
	}
	else if(LEDNumber == 0)
	{
		Chip_GPIO_WritePortBit(LPC_GPIO_PORT, 0, 20, On);
	}
}
...
{
	return Chip_GPIO_ReadPortBit(LPC_GPIO_PORT, 1, 19);
}
